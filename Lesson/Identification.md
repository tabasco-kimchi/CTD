...
http://naver.com? a=b & c=d / **파라미터**

*메타문자(특수문자, 의미문자 = url에서 사용하면 특별한 의마가 생긴다.)*

*의미가 아닌 문자 그 자체로 사용하고 싶을 때*

: company = Bandi & lunice 

—> 서버가 의미문자가 아닌 문자 그대로 인식했으면 좋겠다(escape 시킨다)

**인코딩** %26 (아스키코드)를 사용하여 이스케입 시킨다.
...
1. 안전한 암호 알고리즘을 썼는가?
   ...
   - 비도가 보장되는 것 (ex: DES는 보장이 안됨/ 2DES/3DES->AES ; 차세대 암호화 알고리즘의 스펙; 랜달)
   - - MD5 ㄴㄴ - SHA2이상 등등..
   - - 인코딩으로 구현? ㄴㄴ 시스템 간 개인정보를 이동할 때, url이라던지 등등 인코딩기법으로 쓰면 안전하지 않다.
...
3. 키 안전하게 보관
   ...
   - 접근제어
   - 암호화 처리/ 데이터저장 - 키보관은 물리적/논리적으로 *<u>분리, 격리</u>*  > 분리된 공간에 대해서 적정 통제
